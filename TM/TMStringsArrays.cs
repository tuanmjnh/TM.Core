...
namespace TM.Core.Helpers.TMString
{
    public static class Strings
    {
        ...
        public static string[] Replace(this string[] str, string regex)
        {
            for (int i = 0; i < str.Length; i++)
                str[i] = System.Text.RegularExpressions.Regex.Replace(str[i], regex, "");
            return str;
        }
        ...
        {
            try { return Convert.ToDecimal(val).ToString("N0", Format.Formating.FormatNumber()) + " " + s; } catch (Exception) { return val.ToString(); }
        }
